...
if options.mode in ["train", "refresh"]:
    ...
    for filename in os.listdir(options.raw_input):
        with open(options.raw_input + '/' + filename, "r") as fin:
            instances, _, _ = read_conll(options.raw_input + '/' + filename)
            instances_list.append(instances)
        out_conll_files.append("{}{}".format(model_dir + 'out/', filename))
else:
...
def print_as_conll(goldexamples, pred_targmaps, file):
...
if options.mode in ["train", "refresh"]:
    ...
    for instances_n in instances_list:
        ...
        for ins in instances_n:
            _, prediction = identify_frames(builders, ins.tokens, ins.postags, ins.lu, ins.targetframedict.keys())
