...
if options.mode in ["train", "refresh"]:
    ...
    # print(instances_list[0][0].sentence.postags)
    # print(instances_list[1][0].sentence.postags)
    # print(instances_list[2][0].sentence.postags)
...
def print_as_conll(gold_examples, predicted_target_dict, file):
    """
    Creates a CoNLL object with predicted target and lexical unit.
    Spits out one CoNLL for each LU.
    """
    with codecs.open(file, "w", "utf-8") as conll_file:
...
if options.mode in ["train", "refresh"]:
    ...
    for instances_n in instances_list:

        predictions = []
        for ins in instances_n:
            _, prediction = identify_targets(builders, ins.tokens, ins.postags, ins.lemmas)
            predictions.append(prediction)

        file_to_print = out_conll_files[n_instances]
        sys.stderr.write("Printing output in CoNLL format to {}\n".format(out_conll_files[n_instances]))

        print_as_conll(instances_n, predictions, file_to_print)
        sys.stderr.write("Done!\n")
