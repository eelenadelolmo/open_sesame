...
if options.mode in ["train", "refresh"]:
    ...
    for filename in os.listdir(options.raw_input):
        ...
        # filename[3:] removes 'en_' from input files
        out_conll_files.append("{}{}".format(model_dir + 'out/', filename[3:]))
else:
...
def print_as_conll(golds, pred_targmaps, file):
...
if options.mode in ["train", "refresh"]:
    ...
    for instances_n in instances_list:
        ...
        for ins in instances_n:
            prediction = identify_fes(ins.tokens,
                                      ins.sentence,
                                      ins.targetframedict)
